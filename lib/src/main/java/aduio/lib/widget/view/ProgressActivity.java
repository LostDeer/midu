...
public class ProgressActivity extends RelativeLayout {
    ...
    public void showLoading(boolean isBlack) {
    ...
    private void setLoadingView() {
        if (loadingStateRelativeLayout == null) {
            ...
            mViewBlack = view.findViewById(R.id.v_black);
            if (mIsBlack) {
                String top = SPrefaceUtil.getString(getContext(), Constant.STATUSBARHEIGHT, null);
                Integer integer = Integer.valueOf(top);
                ViewGroup.LayoutParams layoutParams = mViewBlack.getLayoutParams();
                layoutParams.height = integer;
                mViewBlack.setLayoutParams(layoutParams);
                //                mViewBlack.setPadding(0,integer,0,0);
            }
            //            loadingStateProgressBar = (ProgressBar) view.findViewById(loadingStateProgressBar);

            ImageView yun = (ImageView) view.findViewById(R.id.yun);
            ImageView niao = (ImageView) view.findViewById(R.id.niao);

            ObjectAnimator objectAnimatorA = ObjectAnimator.ofFloat(yun, "Alpha", 0.3f, 1, 0.3f);
            objectAnimatorA.setRepeatMode(ValueAnimator.RESTART);
            objectAnimatorA.setDuration(2000);
            objectAnimatorA.setRepeatCount(-1);//重复执行

            ObjectAnimator objectAnimatorB = ObjectAnimator.ofFloat(yun, "TranslationX", 80, -200);
            objectAnimatorB.setRepeatMode(ValueAnimator.RESTART);
            objectAnimatorB.setDuration(2000);
            objectAnimatorB.setRepeatCount(-1);

            ObjectAnimator animatorC = ObjectAnimator.ofFloat(niao, "TranslationY", -15, 0, 15);
    ...
    private void setErrorView() {
        if (errorStateRelativeLayout == null) {
            ...
            View fanhui = view.findViewById(R.id.iv_fanhui);

            if (className.equals("MainActivity") || className.equals("WelcomeActivity")) {
            ...
            errorStateTitleTextView.setTextColor(getContext().getResources().getColor(R.color.money449));
